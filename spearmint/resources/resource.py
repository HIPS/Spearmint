...
# Parses the config dict and gets out the resource objects
def parse_resources_from_config(config):

    # If the user did not explicitly specify resources
    if "resources" not in config:
        default_resource_name = 'Main'
        tasks = parse_tasks_in_resource_from_config(config, default_resource_name)
        return {default_resource_name : resource_factory(default_resource_name, tasks, config)}

    # If resources are specified
    else:
        resources = dict()
        for resource_name, resource_opts in config["resources"].iteritems():
            tasks = parse_tasks_in_resource_from_config(config, resource_name)
            resources[resource_name] = resource_factory(resource_name, tasks, resource_opts)
        return resources
...
def resource_factory(name, tasks, config):
        ...
        return Resource(name, tasks, scheduler_object, scheduler_class, max_concurrent, max_finished_jobs)
