...
class productCov:
    ...
    def kernel_grad(self, x1, x2=None, r=None):
        (K, dK) = self.kernel(x1, x2=x2, grad=True)
        return dK

    def sample_hypers(self, logprob_fun, params, slice_sample_fun):
        for k in self.kernels:
            k.sample_hypers(logprob_fun, params[k.name], slice_sample_fun)

    def to_hypers(self, hypers):
        for kernel in self.kernels:
            if kernel.name not in hypers:
                hypers[kernel.name] = {}
            kernel.to_hypers(hypers[kernel.name])

    def from_hypers(self, hypers):
        for kernel in self.kernels:
            kernel.from_hypers(hypers[kernel.name])

    def set_sample(self, seed):
        for kernel in self.kernels:
            kernel.set_sample(seed)

    def append_sample(self):
        for kernel in self.kernels:
            kernel.append_sample()
