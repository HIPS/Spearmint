...
class Transformer(object):
    ...
    def add_layer(self, *layer_transformations):
        ...
        remaining_inds = set(range(num_input_dims))
        remaining_inds.difference_update(set([i for inds in t_inds for i in inds]))
        self.layer_remaining_inds.append(list(remaining_inds))

        output_dims = [t.output_num_dims() for t in transformations]
        self.layer_output_dims.append(sum(output_dims) + len(remaining_inds))
        ...
        if len(layer_transformations) == 1:
            return output_inds[0]
        else:
            return output_inds

    def validate_layer(self, layer_inds):
        counts = defaultdict(int)

        for inds in layer_inds:
            for i in inds:
                counts[i] += 1
    ...
    def forward_pass(self, inputs):
        ...
        for transformations, t_inds, remaining_inds, output_num_dims in zip(self.layer_transformations,
            ...
            layer_out = np.zeros((prev_layer.shape[0], output_num_dims))
            i = 0
